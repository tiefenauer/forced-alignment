...
def fake_data(num_examples, num_features, num_labels, min_size=10, max_size=100):
    rl_corpus_root = os.path.join('E:\\', 'readylingua-corpus')
    rl_corpus = load_corpus(rl_corpus_root)
    corpus_entry = rl_corpus[0]
    speech_segments = corpus_entry.speech_segments_not_numeric[:num_examples]
    inputs = np.asarray([seg.mfcc() for seg in speech_segments])
    # inputs = np.asarray([seg.spectrogram().T for seg in speech_segments])
    labels = np.asarray([encode(seg.text) for seg in speech_segments])
...
with tf.Session(graph=graph) as session:
    ...
    for curr_epoch in range(num_epochs):
        ...
        for batch in range(num_batches_per_epoch):
            ...
            # batch_train_inputs.shape = (2,) : contains 2 samples of shape (seq_len, num_features=13)
            batch_train_inputs = train_inputs[indexes]
            # Padding input to max_time_step of this batch
            # batch_train_inputs.shape = (batch_size, max(seq_len), num_features)
            # batch_train_seq_len is an array containing the sequence lenghts
            ...
            for i, prediction_enc in enumerate(dense_decoded):
                ground_truth = decode(train_targets[i + batch * batch_size])
